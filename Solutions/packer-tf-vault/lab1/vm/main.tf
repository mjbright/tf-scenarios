...
output  "ssh_connect"       { value = "ssh -i key.pem ubuntu@${ aws_instance.example.public_ip } uptime" }
output  "ssh_status"       { value = "ssh -i key.pem ubuntu@${ aws_instance.example.public_ip } 'echo $(date): $(hostname) $(hostname -i) $(uptime)' " }
