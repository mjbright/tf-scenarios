locals {
  ...
  location = var.obj_resource_group.location
}
...
resource "azurerm_public_ip" "pip" {
  ...
  resource_group_name = var.obj_resource_group.name
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = lower("proxigrp-${var.virtual_machine_name}-${local.location}")
  resource_group_name = var.obj_resource_group.name
  location            = local.location
  tags                = merge({ "ResourceName" = lower("proxigrp-${var.virtual_machine_name}-${local.location}") }, var.tags, )
